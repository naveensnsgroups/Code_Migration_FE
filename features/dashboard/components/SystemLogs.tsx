    ...
    <div className="h-48 border-t border-zinc-800 bg-zinc-950 flex flex-col shrink-0 z-50">
      <div className="h-8 flex items-center justify-between px-4 bg-zinc-900/50 border-b border-zinc-800/50">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Terminal — System Logs</span>
        </div>
        <button 
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 font-mono text-[11px] space-y-1 selection:bg-yellow-400/20">
        {logs.map((log) => (
          <div key={log.id} className="flex gap-3 animate-in fade-in slide-in-from-left-2 duration-300">
            ...
            <span className={`
              ...
              ${log.type === 'ai' ? 'text-blue-400 italic' : 'text-zinc-300'}
            `}>
              {log.type === 'ai' && (
                <span className="mr-2 font-bold not-italic inline-flex items-center gap-1.5 translate-y-[1px]">
